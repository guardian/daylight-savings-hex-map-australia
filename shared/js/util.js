...
const timestamp = (inputdate) => {
  var timestamp = `${inputdate.getUTCHours() < 10 ? "0": ""}${inputdate.getUTCHours()}:${inputdate.getMinutes() < 10 ? "0" : ""}${inputdate.getMinutes()} UTC ${inputdate.getDate()} ${inputdate.toLocaleString("en-gb",{month: 'short'})} ${inputdate.getFullYear()}`;
  return timestamp;
}
...
let isPreview = false;
  ...
  if (isPreview) {
    return "https://interactive.guim.co.uk/docsdata-test/1cgovw4pP4vBQTU5g6wWC7-2XygrZoCx_85DQg33Br9M.json"
  }
  else {
    return "https://interactive.guim.co.uk/docsdata/1cgovw4pP4vBQTU5g6wWC7-2XygrZoCx_85DQg33Br9M.json"
  }
